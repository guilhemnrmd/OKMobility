...
async function hydrateAgencyBrandingFromUrl() {
    ...
    try {
        ...
        if (payload?.valid) {
            ...
            if (s.logoUrl) {
                ...
                if (logoContainer) {
                    logoContainer.innerHTML = ''; // Hide OKM default logo if no custom logo provided
                }
            }
            ...
            const DEFAULT_CLIENT_FIELDS = [
                { id: "address_block", type: "address_block", icon: "bx-map", required: true, requireCountry: true },
                { id: "temp_address_group", type: "toggle_group", icon: "bx-list-plus", children: [{ id: "temp_address_block", type: "address_block", icon: "bx-map-pin", required: true, requireCountry: false }] },
                { id: "phone", type: "tel", icon: "bx-phone", required: true },
                { id: "phone2_group", type: "toggle_group", icon: "bx-list-plus", children: [{ id: "phone2", type: "tel", icon: "bx-phone", required: true }] },
                { id: "email", type: "email", icon: "bx-envelope", required: true }
            ];

            const fieldsToRender = (Array.isArray(s.fields) && s.fields.length > 0) ? s.fields : DEFAULT_CLIENT_FIELDS;
            state.activeFields = fieldsToRender;
    ...
    // Fallback if network fails: always render the default fields since HTML is empty
    if (!state.activeFields || state.activeFields.length === 0) {
        ...
        state.activeFields = DEFAULT_CLIENT_FIELDS;
        renderDynamicFields(DEFAULT_CLIENT_FIELDS);
        reinitFormBindings();
    }
}
